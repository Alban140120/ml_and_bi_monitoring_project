...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE NBOChurn (
# MAGIC SELECT i.*
...
# MAGIC %md
# MAGIC ### Création de la variable à expliquer
...
events = df.filter(F.col("EVENT_TYPE") == "CHURN REQUEST") \
...
# MAGIC %md
# MAGIC ### Création modèle de machine learning (prédiction TARGET_CHURN)
...
class XGBProbaWrapper(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input):
        # Retourne uniquement la proba de la classe "1"
        return self.model.predict_proba(model_input)[:, 1]

wrapped_model = XGBProbaWrapper(model_churn)

# 5. Inférer la signature (basée sur les proba)
signature = infer_signature(X_train_res, model_churn.predict_proba(X_train_res)[:, 1])

# 6. Log du modèle avec MLflow (en PyFunc)
...
# 7. Évaluation locale pour vérif
...
df_final["PREDICTION_CHURN"] = y_pred
df_final["THRESHOLD_USED"] = threshold

# Réorganisation des colonnes pour lisibilité
ordered_cols = (
    ["CUST_NUM", "YEAR", "MONTH", target_col, "PREDICTION_CHURN", "y_proba", "THRESHOLD_USED"]
...
model_full_name = "amavla_dev_central_adbw.default.churn_model"
...
#test_pd['TARGET_CHURN'].value_counts()
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS nboTrainChurn;

# COMMAND ----------

# Sauver le dataframe au format delta table natif de Databricks
df_final.write.format("delta").saveAsTable("nboTrainChurn")

# COMMAND ----------

# MAGIC %sql
# MAGIC DESCRIBE nboTrainChurn;
...
# MAGIC SELECT *
# MAGIC FROM nboTrainChurn;

# COMMAND ----------

# MAGIC %sql
# MAGIC select sum(TARGET_CHURN), sum(PREDICTION_CHURN)
# MAGIC from nboTrainChurn
# MAGIC where TARGET_CHURN = 1;
