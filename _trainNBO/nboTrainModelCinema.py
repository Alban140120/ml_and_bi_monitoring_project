...
# MAGIC %md
# MAGIC ### Jointure entre les 6 delta tables

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE NBOCinema (
# MAGIC SELECT i.*
...
events = df.filter(F.col("EVENT_TYPE") == "CINEMA REQUEST") \
...
# MAGIC %md
# MAGIC ### Création modèle de machine learning (prédiction TARGET_CINEMA)
...
model_cinema.fit(X_train_res, y_train_res)

import shap

# Explainer SHAP
explainer = shap.TreeExplainer(model_cinema)
...
# 4. Créer un wrapper qui renvoie les probabilités
...
wrapped_model = XGBProbaWrapper(model_cinema)

# 5. Inférer la signature (probabilités entre 0 et 1)
signature = infer_signature(X_train_res, model_cinema.predict_proba(X_train_res)[:, 1])

# 6. Log du modèle dans MLflow (pyfunc, pas xgboost)
mlflow.pyfunc.log_model(
    artifact_path="model_cinema",
    python_model=wrapped_model,
    registered_model_name="cinema_model",
    signature=signature
)

# 7. Évaluation locale pour contrôle
y_proba = model_cinema.predict_proba(X_test)[:, 1]
...
df_final["PREDICTION_CINEMA"] = y_pred
df_final["THRESHOLD_USED"] = threshold

# Réorganisation des colonnes
ordered_cols = (
    ["CUST_NUM", "YEAR", "MONTH", target_col, "PREDICTION_CINEMA", "y_proba", "THRESHOLD_USED"]
...
shap_spark.write \
    .format("delta") \
    .mode("append") \
    .saveAsTable("feature_importance_cinema")
...
model_full_name = "amavla_dev_central_adbw.default.cinema_model"
...
booster = model_cinema.get_booster()
...
# MAGIC %sql
# MAGIC DROP TABLE IF EXISTS nboTrainCinema;

# COMMAND ----------

# Sauver le dataframe au format delta table natif de Databricks
df_final.write.format("delta").saveAsTable("nboTrainCinema")
...
# MAGIC SELECT * 
# MAGIC FROM nboTrainCinema;

# COMMAND ----------

# MAGIC %sql
# MAGIC select sum(target_cinema), sum(prediction_cinema)
# MAGIC from nboTrainCinema
# MAGIC where TARGET_CINEMA = 1;
